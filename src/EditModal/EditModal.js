...
export const EditModal = ({
  ...
  const oldTitle = listOfSeans
    .filter((item) => item.id === idSelectedSeans)
    .map((item) => item.title)
    .toString();
  const oldTime = listOfSeans
    .filter((item) => item.id === idSelectedSeans)
    .map((item) => item.time)
    .toString();
  const oldArea = listOfSeans
    .filter((item) => item.id === idSelectedSeans)
    .map((item) => item.area)
    .toString();
    ...
    >
      ...
      <div className="editModalContainer">
        ...
        <form className="formEditModal" onSubmit={changeDatas}>
          <input
            className="inputEditModal inputEditModalTitle"
            required
            type="text"
          ...
          />
          <input
            className="inputEditModal inputEditModalTime"
            required
            type="time"
            value={input.time}
          ...
          />
          <input
            className="inputEditModal inputEditModalArea"
            type="number"
            min="1"
            max="3"
            placeholder={oldArea}
